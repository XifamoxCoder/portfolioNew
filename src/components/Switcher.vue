...
import {useDarkThemeStore} from "../stores/DarkThemeStore.ts";

const darkThemeStore = useDarkThemeStore()

// Инициализируем реактивную переменную для состояния переключателя

// Обработчик изменения состояния переключателя
const toggleDarkMode = () => {
  darkThemeStore.darkTheme = !darkThemeStore.darkTheme
  darkThemeStore.darkTheme = darkThemeStore.darkTheme.toString()
  // Сохраняем состояние в локальное хранилище
  localStorage.setItem('darkMode', darkThemeStore.darkTheme);
};

// Загружаем состояние из локального хранилища при монтировании компонента
onMounted(() => {
  const savedDarkMode = localStorage.getItem('darkMode');
  darkThemeStore.darkTheme = savedDarkMode === 'true';
  console.log(darkThemeStore.darkTheme)
});
</script>

<template>
  <label class="switch">
    <input class="switch__input" type="checkbox" :checked="darkThemeStore.darkTheme" role="switch" @change="toggleDarkMode">
...
.switch__icon--dark {
  opacity: 0;
  transform: translateX(-0.65em) rotate(30deg) scale(0.75);
